...
for k = 1:length(unique_keys)
    ...
    % ------------------------------------------------------------------
    %  Average betas (and SD) across surgeries (NaN for missing)
    % ------------------------------------------------------------------
    ...
    beta_sd  = std(beta_all, 0, 3, 'omitnan');  % SD across surgeries (dim 3), normalised by N-1

    % Create channel-level mean table
    avg_table = array2table(beta_avg, ...
        'RowNames', common_conditions, ...
        'VariableNames', common_channels);

    % Create channel-level SD table (columns prefixed with 'SD_')
    sd_ch_colnames = cellfun(@(x) ['SD_' x], common_channels, 'UniformOutput', false);
    sd_table = array2table(beta_sd, ...
        'RowNames', common_conditions, ...
        'VariableNames', sd_ch_colnames);

    % ------------------------------------------------------------------
    %  SAVE: Channel-level averaged betas (mean + SD in one file)
    % ------------------------------------------------------------------
    % Interleave mean and SD columns: ch1_mean, ch1_SD, ch2_mean, ch2_SD ...
    combined_ch_data  = NaN(n_conds, n_ch * 2);
    combined_ch_names = cell(1, n_ch * 2);
    for c = 1:n_ch
        combined_ch_data(:, 2*c-1) = beta_avg(:, c);
        combined_ch_data(:, 2*c)   = beta_sd(:, c);
        combined_ch_names{2*c-1}   = common_channels{c};
        combined_ch_names{2*c}     = ['SD_' common_channels{c}];
    end
    combined_ch_table = array2table(combined_ch_data, ...
        'RowNames', common_conditions, ...
        'VariableNames', combined_ch_names);

    out_ch = fullfile(output_dir, sprintf('grouped_betas_avg_%s_%s.csv', signal, key));
    writetable(combined_ch_table, out_ch, 'WriteRowNames', true);
    fprintf('  Saved channel-level (mean + SD): %s\n', out_ch);
    ...
    region_sd  = NaN(n_conds, n_regions);
    ...
    for r = 1:n_regions
        ...
        if ~isempty(col_idx)
            ...
            % SD across region channels (spatial variability within the region)
            region_sd(:, r)  = std(beta_avg(:, col_idx), 0, 2, 'omitnan');
    ...
    % Interleave mean and SD columns for region table
    combined_reg_data  = NaN(n_conds, n_regions * 2);
    combined_reg_names = cell(1, n_regions * 2);
    for r = 1:n_regions
        combined_reg_data(:, 2*r-1) = region_avg(:, r);
        combined_reg_data(:, 2*r)   = region_sd(:, r);
        combined_reg_names{2*r-1}   = region_labels{r};
        combined_reg_names{2*r}     = ['SD_' region_labels{r}];
    end
    combined_reg_table = array2table(combined_reg_data, ...
        'RowNames', common_conditions, ...
        'VariableNames', combined_reg_names);

    % ------------------------------------------------------------------
    %  SAVE: Region-level averaged betas (mean + SD in one file)
    % ------------------------------------------------------------------
    out_reg = fullfile(output_dir, sprintf('grouped_betas_region_%s_%s.csv', signal, key));
    writetable(combined_reg_table, out_reg, 'WriteRowNames', true);
    fprintf('  Saved region-level (mean + SD): %s\n', out_reg);
    ...
    for g = 1:n_conds
        for c = 1:n_ch
            ...
            row_data = {surgeon, joint, tech, key, common_conditions{g}, ...
                common_channels{c}, ch_nums(c), beta_avg(g, c), beta_sd(g, c), n_surgeries};
            master_channel = [master_channel; cell2table(row_data, ...
                'VariableNames', {'Surgeon','Joint','Tech','Group_Key', ...
                'Condition','Channel','Channel_Num','Beta','Beta_SD','N_Surgeries'})];
        end
    end
    ...
    for g = 1:n_conds
        for r = 1:n_regions
            ...
            row_data = {surgeon, joint, tech, key, common_conditions{g}, ...
                regions(r).name, regions(r).label, region_avg(g, r), region_sd(g, r), ...
                region_ch_count(r), n_surgeries};
            master_region = [master_region; cell2table(row_data, ...
                'VariableNames', {'Surgeon','Joint','Tech','Group_Key', ...
                'Condition','Region','Region_Label','Beta','Beta_SD','N_Channels','N_Surgeries'})];
        end
    end
    ...
    %  DISPLAY PREVIEW
    % ------------------------------------------------------------------
    fprintf('\n  --- Region beta preview (mean) for %s ---\n', key);
    disp(array2table(region_avg, 'RowNames', common_conditions, 'VariableNames', region_labels));
    fprintf('  --- Region beta preview (SD) for %s ---\n', key);
    disp(array2table(region_sd,  'RowNames', common_conditions, 'VariableNames', region_labels));
end
...
fprintf('\nNote on SD interpretation:\n');
fprintf('  Channel-level Beta_SD : SD across surgeries (temporal variability)\n');
fprintf('  Region-level  Beta_SD : SD across channels within region (spatial variability)\n');
fprintf('\nRegion definitions:\n');
for r = 1:n_regions
